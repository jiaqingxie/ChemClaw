...
parser.add_argument(
    ...
    default=False,
    help="Show the spectrum name as the plot title.",
)
...
if args.file2 and args.name2:
    ...
    plot_title = args.name1 + " matched by " + args.name2.split("/")[0] if args.show_title else None
    sv.plot_spectrum(s1, s2, title=plot_title, out=args.out) #,annotate=args.annotate, peptide=args.peptide, charge=args.charge, font_size=args.fontsize)
else:
    plot_title = args.name1 if args.show_title else None
    sv.plot_spectrum(s1, title=plot_title, out=args.out, show=True)#, font_size=args.fontsize)